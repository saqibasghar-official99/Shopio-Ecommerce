...
import { Plus, Search, Pencil, Trash2, X, Upload, Link as LinkIcon } from 'lucide-react';
...
export default function AdminProductsPage() {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const raw: MongoProduct[] = json.data || json.products || (Array.isArray(json) ? json : []);
  ...
  const fetchCategories = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        const json = await res.json();
        const raw: MongoCategory[] = json.data || (Array.isArray(json) ? json : []);
  ...
  const addVariantGroup = () => {
    setForm((prev) => ({ ...prev, variants: [...prev.variants, { label: '', options: [] }] }));
  };
  ...
  const updateVariantGroup = (idx: number, field: 'label' | 'options', value: string) => {
    setForm((prev) => ({
      ...prev,
      variants: prev.variants.map((v, i) =>
        i === idx
          ? field === 'options'
            ? { ...v, options: value.split(',').map((o) => o.trim()).filter(Boolean) }
            : { ...v, label: value }
          : v
      ),
    }));
  ...
  const updateSpecification = (idx: number, field: 'key' | 'value', value: string) => {
    setForm((prev) => ({
      ...prev,
      specifications: prev.specifications.map((s, i) => i === idx ? { ...s, [field]: value } : s),
    }));
  };
  ...
  const columns = [
    ...
    },
    {
      key: 'price',
      label: 'Price',
      render: (row: Record<string, unknown>) => formatCurrency(row.price as number),
    },
    {
      key: 'stock',
      label: 'Stock',
      render: (row: Record<string, unknown>) => {
        ...
        return (
          <span className={cn('font-medium', stock <= 0 ? 'text-red-600' : stock <= 10 ? 'text-yellow-600' : 'text-green-600')}>
            {stock}
          </span>
        );
      },
    },
    {
      ...
      label: 'Specs',
      render: (row: Record<string, unknown>) => {
        const specs = row.specifications as { key: string; value: string }[] | undefined;
    ...
    },
    {
      ...
      label: 'Status',
      render: (row: Record<string, unknown>) => (
        <Badge variant={row.is_active ? 'default' : 'secondary'} className={cn('text-xs', row.is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500')}>
      ...
      render: (row: Record<string, unknown>) => {
        ...
        return (
          <div className="flex items-center gap-1">
            <button onClick={() => openEditDialog(product)} className="rounded p-1 text-gray-400 hover:bg-gray-100 hover:text-gray-600">
              <Pencil className="h-3.5 w-3.5" />
            </button>
            <button onClick={() => handleDelete(product)} disabled={deletingId === product.id} className="rounded p-1 text-gray-400 hover:bg-red-50 hover:text-red-600">
        ...
        <CardContent className="flex flex-wrap items-end gap-3 p-4">
          <div className="space-y-1">
            ...
            <div className="relative">
              ...
              <Input placeholder="Product name..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} className="h-8 w-48 pl-8 text-xs" />
            </div>
          </div>
          <div className="space-y-1">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    setForm((prev) => ({ ...prev, name, slug: editingSlug ? prev.slug : slugify(name) }));
                  }}
              ...
              <div className="space-y-1">
                ...
                <Input value={form.slug} onChange={(e) => setForm((prev) => ({ ...prev, slug: e.target.value }))} className="h-8 text-xs" placeholder="product-slug" />
              </div>
            </div>
            ...
            <div className="space-y-1">
              ...
              <Textarea value={form.description} onChange={(e) => setForm((prev) => ({ ...prev, description: e.target.value }))} className="text-xs" rows={3} placeholder="Product description" />
            </div>

            <div className="space-y-1">
              <Label className="text-sm text-gray-700">Short Description</Label>
              <Textarea value={form.short_description} onChange={(e) => setForm((prev) => ({ ...prev, short_description: e.target.value }))} className="text-xs" rows={2} placeholder="Brief product summary" />
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="space-y-1">
                ...
                <Input type="number" value={form.price} onChange={(e) => setForm((prev) => ({ ...prev, price: e.target.value }))} className="h-8 text-xs" placeholder="0.00" step="0.01" />
              </div>
              <div className="space-y-1">
                <Label className="text-sm text-gray-700">Compare Price</Label>
                <Input type="number" value={form.compare_price} onChange={(e) => setForm((prev) => ({ ...prev, compare_price: e.target.value }))} className="h-8 text-xs" placeholder="0.00" step="0.01" />
              </div>
              <div className="space-y-1">
                <Label className="text-sm text-gray-700">Cost</Label>
                <Input type="number" value={form.cost} onChange={(e) => setForm((prev) => ({ ...prev, cost: e.target.value }))} className="h-8 text-xs" placeholder="0.00" step="0.01" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="space-y-1">
                ...
                <Input type="number" value={form.stock} onChange={(e) => setForm((prev) => ({ ...prev, stock: e.target.value }))} className="h-8 text-xs" placeholder="0" />
              </div>
              <div className="space-y-1">
                <Label className="text-sm text-gray-700">SKU</Label>
                <Input value={form.sku} onChange={(e) => setForm((prev) => ({ ...prev, sku: e.target.value }))} className="h-8 text-xs" placeholder="SKU-001" />
              </div>
              <div className="space-y-1">
                <Label className="text-sm text-gray-700">Weight (g)</Label>
                <Input type="number" value={form.weight} onChange={(e) => setForm((prev) => ({ ...prev, weight: e.target.value }))} className="h-8 text-xs" placeholder="0" />
              </div>
            </div>
            ...
            <div className="space-y-1">
              ...
              <Input value={form.tags} onChange={(e) => setForm((prev) => ({ ...prev, tags: e.target.value }))} className="h-8 text-xs" placeholder="tag1, tag2, tag3" />
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] text-gray-400">Up to 5 images. First image is the main image.</span>
              </div>
              ...
              <div className="rounded-md border border-dashed border-gray-300 p-3 space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  />
                  <Button type="button" onClick={addImageByURL} variant="outline" className="h-8 text-xs shrink-0" disabled={form.images.length >= 5}>
                    Add
                  </Button>
                </div>
              </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {form.images.map((url, idx) => (
                    <div key={idx} className="relative">
                      ...
                      {idx === 0 && (
                        <span className="absolute bottom-0 left-0 right-0 bg-green-600 text-white text-[8px] text-center py-0.5 rounded-b font-medium">Main</span>
                      )}
                      <button
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <Button type="button" onClick={addSpecification} variant="outline" className="h-7 text-xs">
              ...
              {form.specifications.map((spec, idx) => (
                <div key={idx} className="flex items-center gap-2">
                  ...
                  />
                  <button onClick={() => removeSpecification(idx)} className="text-gray-400 hover:text-red-500 shrink-0">
                    <Trash2 className="h-3.5 w-3.5" />
                  </button>
                </div>
              ))}
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <Button type="button" onClick={addVariantGroup} variant="outline" className="h-7 text-xs">
                  <Plus className="mr-1 h-3 w-3" />
                  Add Variant Group
                </Button>
              </div>
              {form.variants.map((variant, idx) => (
                <div key={idx} className="rounded border p-3 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-medium text-gray-700">Variant {idx + 1}</span>
                    <button onClick={() => removeVariantGroup(idx)} className="text-gray-400 hover:text-red-500">
                      <Trash2 className="h-3.5 w-3.5" />
                    </button>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <Input value={variant.label} onChange={(e) => updateVariantGroup(idx, 'label', e.target.value)} className="h-8 text-xs" placeholder="Label (e.g. Size)" />
                    <Input value={variant.options.join(', ')} onChange={(e) => updateVariantGroup(idx, 'options', e.target.value)} className="h-8 text-xs" placeholder="Options (e.g. S, M, L)" />
                  </div>
                </div>
              ))}
            </div>
