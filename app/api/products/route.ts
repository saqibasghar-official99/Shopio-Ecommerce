...
const LIST_PROJECTION = {
  ...
  // Keep images for now
  // TODO: slice to 1 once thumbnails are available
  images: 1,
};
...
export async function GET(request: NextRequest) {
  try {
    ...
    /* ========================================================
       ...
       Example:
       /api/products?badge=FLASH

       Combined example:
       /api/products?tag=Deals&badge=FLASH
    ======================================================== */
    ...
    /* ========================================================
       ...
       Count + products execute in parallel.
    ======================================================== */
    ...
    /* ========================================================
       ...
       Currently disabled because the project is still using
       the original images.

       When image transformation is ready, replace:

       const data = rawData;

       with the transformProductImages mapping below.
    ======================================================== */
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      ...
      !body.category_id.match(
        /^[0-9a-fA-F]{24}$/
      )
    ) {
      const cat = await Category.findOne({
        slug: body.category_id,
      })
        .select('_id')
        .lean<{ _id: unknown }>();
